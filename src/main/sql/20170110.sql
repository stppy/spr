CREATE OR REPLACE VIEW pivot_indicadores AS 
 SELECT nivel.id AS nivel_id,
	...
	ind.cobertura_geografica_alcance AS indicador_cobertura_geografica,
	ind.nivel_despliegue_geografico AS indicador_despliegue_demografico,
	ind.cobertura_demografica_alcance AS indicador_cobertura_demografica,
   ...
   FROM entidad ent
	...
	JOIN indicador ind ON ind.nivel = nivel.id AND ind.entidad = ent.id AND ind.objetivo_id = ohi.objetivo_id 
	JOIN unidad_medida um ON ind.unidad_medida_id = um.id
	JOIN meta ON meta.nivel = ind.nivel AND meta.entidad = ind.entidad AND meta.indicador_id = ohi.indicador_id
     
  WHERE NOT nivel.borrado AND NOT ent.borrado AND NOT ur.borrado AND NOT obj.borrado AND NOT ind.borrado AND NOT meta.borrado 
  AND NOT um.borrado 
